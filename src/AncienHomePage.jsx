...
function HomePage() {
  ...
  // État pour l'animation de transition (ex: "slide-in-right", "slide-out-left", etc.)
  ...
  // Retour ("goBack") : action du bouton central (flèche up)
  // Pour la vue des compétences, si une catégorie est affichée, revenir à la liste des catégories.
  const goBack = () => {
    if (
      ...
      triggerTransition("down", () => setCurrentCompetenceCategory(null));
    } else if (currentMenu === "coups-critiques" && currentCritDetail) {
      triggerTransition("down", () => setCurrentCritDetail(null));
    } else if (currentMenu === "post-bataille" && currentPostBattleDetail) {
      // Pour la section post-bataille hors compétences
      if (currentCompetenceCategory) {
        triggerTransition("down", () => setCurrentCompetenceCategory(null));
      } else {
        triggerTransition("down", () => setCurrentPostBattleDetail(null));
      }
  ...
  // Passage au détail suivant ("goNext") : déclenché par swipe gauche ou la flèche droite
  const goNext = () => {
    // Navigation dans la liste des compétences (dans post-bataille, si on affiche "liste_compétences")
    if (
      currentMenu === "post-bataille" &&
      currentPostBattleDetail === "liste_compétences"
    ) {
      ...
      if (!currentCompetenceCategory) {
        triggerTransition("left", () => setCurrentCompetenceCategory(keys[0]));
      } else {
        const currentIndex = keys.indexOf(currentCompetenceCategory);
        const nextIndex = (currentIndex + 1) % keys.length;
        triggerTransition("left", () =>
          setCurrentCompetenceCategory(keys[nextIndex])
        );
      }
    }
    // Navigation dans les coups critiques
      ...
      if (!currentCritDetail) {
        triggerTransition("left", () => setCurrentCritDetail(keys[0]));
      } else {
        const currentIndex = keys.indexOf(currentCritDetail);
        const nextIndex = (currentIndex + 1) % keys.length;
        triggerTransition("left", () =>
          setCurrentCritDetail(keys[nextIndex])
        );
      }
    }
    // Navigation dans la section post-bataille hors compétences
      ...
      if (!currentPostBattleDetail) {
        triggerTransition("left", () => setCurrentPostBattleDetail(keys[0]));
      } else if (currentPostBattleDetail !== "liste_compétences") {
        const currentIndex = keys.indexOf(currentPostBattleDetail);
        const nextIndex = (currentIndex + 1) % keys.length;
        triggerTransition("left", () =>
          setCurrentPostBattleDetail(keys[nextIndex])
        );
      }
    }
  };

  // Passage au détail précédent ("goPrevious") : déclenché par swipe droite ou la flèche gauche
  const goPrevious = () => {
    // Navigation dans la liste des compétences
    if (
      currentMenu === "post-bataille" &&
      currentPostBattleDetail === "liste_compétences"
    ) {
      ...
      if (currentCompetenceCategory) {
        ...
        triggerTransition("right", () =>
          setCurrentCompetenceCategory(keys[prevIndex])
        );
      }
    }
    // Navigation dans les coups critiques
      ...
      const prevIndex = (currentIndex - 1 + keys.length) % keys.length;
      triggerTransition("right", () =>
        setCurrentCritDetail(keys[prevIndex])
      );
    }
    // Navigation dans la section post-bataille hors compétences
    else if (currentMenu === "post-bataille") {
      const keys = Object.keys(postBattleDetails);
      if (
        currentPostBattleDetail &&
        currentPostBattleDetail !== "liste_compétences"
      ) {
        const currentIndex = keys.indexOf(currentPostBattleDetail);
        const prevIndex = (currentIndex - 1 + keys.length) % keys.length;
        triggerTransition("right", () =>
          setCurrentPostBattleDetail(keys[prevIndex])
        );
      }
    }
  };
  ...
  // Nouveaux mappings :
  // * "left" : pour le passage au détail suivant (contenu sort vers la gauche, nouveau entre par la droite)
  // * "right": pour le détail précédent (contenu sort vers la droite, nouveau entre par la gauche)
  // * "down" : pour le retour (contenu sort vers le bas, nouveau entre par le haut)
  ...
  // --- Gestion des swipes ---
  // Seul le swipe horizontal est pris en compte, afin de permettre le scroll vertical
  ...
  const handleTouchMove = (e) => {
    // Ne pas empêcher le scroll vertical
  };
      ...
      case "home":
        return (
          <div className="main-content">
            <button className="menu-button" onClick={() => triggerTransition("left", () => setCurrentMenu("regles"))}>Règles</button>
            <button className="menu-button" onClick={() => triggerTransition("left", () => setCurrentMenu("coups-critiques"))}>Coups Critiques</button>
            <button className="menu-button" onClick={() => triggerTransition("left", () => setCurrentMenu("post-bataille"))}>Post-Bataille</button>
            <button className="menu-button" onClick={() => triggerTransition("left", () => setCurrentMenu("objets-equipements"))}>Objets et Équipements</button>
            <button className="menu-button" onClick={() => triggerTransition("left", () => setCurrentMenu("sorts-prieres"))}>Sorts et Prières</button>
            <button className="menu-button" onClick={() => triggerTransition("left", () => setCurrentMenu("regles-speciales"))}>Règles Spéciales</button>
            <button className="menu-button" onClick={() => triggerTransition("left", () => setCurrentMenu("mes-bandes"))}>Mes Bandes</button>
          </div>
        );
  ...
  const renderPostBattleMenu = () => (
    <div className="main-content">
      {Object.keys(postBattleDetails).map((key) => (
        <button key={key} className="menu-button" onClick={() => triggerTransition("left", () => setCurrentPostBattleDetail(key))}>
  ...
  const renderCriticalHitsMenu = () => (
    <div className="main-content">
      {Object.keys(criticalHitsDetails).map((key) => (
        <button key={key} className="menu-button" onClick={() => triggerTransition("left", () => setCurrentCritDetail(key))}>
        ...
        <div className="compet-menu" key="liste-competences">
          {Object.keys(competencesDetails).map((key) => (
            <button
              ...
              onClick={() => triggerTransition("left", () => setCurrentCompetenceCategory(key))}
            >
  ...
  return (
    ...
    >
      ...
      {/* Nav Bar toujours visible avec trois flèches :
          - À gauche : flèche qui déclenche goPrevious (équivalent au swipe droit)
          - Au centre : flèche qui déclenche goBack (retour)
          - À droite : flèche qui déclenche goNext (équivalent au swipe gauche)
      */}
